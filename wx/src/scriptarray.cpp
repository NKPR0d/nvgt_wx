...
//
// nvgt_wx vendoring note: when the plugin links AngelScript via
// ANGELSCRIPT_DLL_MANUAL_IMPORT (the default for NVGT plugins),
// `asAllocMem` and `asFreeMem` are function-pointer *variables*,
// not real functions, and they are still null when this translation
// unit's static initializers run — they only get populated when
// `prepare_plugin()` walks the function-pointer table inside
// `plugin_main()`. Capturing them here at static-init time would
// freeze a null pointer into `userAlloc`/`userFree`, so we initialize
// to `nullptr` and route every allocation through a wrapper that
// resolves the current `asAllocMem`/`asFreeMem` value at call time.
static asALLOCFUNC_t userAlloc = nullptr;
static asFREEFUNC_t  userFree  = nullptr;

static inline void* user_alloc(size_t size)
{
	return (userAlloc ? userAlloc : asAllocMem)(size);
}

static inline void user_free(void* mem)
{
	(userFree ? userFree : asFreeMem)(mem);
}
...
{
	// Allocate the memory
	void *mem = user_alloc(sizeof(CScriptArray));
	if( mem == 0 )
	{
...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "void extend(const array<T>& arr)", asMETHODPR(CScriptArray, InsertLast, (const CScriptArray &), void), asCALL_THISCALL); assert(r >= 0);
	...
	r = engine->RegisterObjectMethod("array<T>", "void sort_ascending()", asMETHODPR(CScriptArray, SortAsc, (), void), asCALL_THISCALL); assert( r >= 0 );
...
void CScriptArray::Reserve(asUINT maxElements)
{
	...
	SArrayBuffer *newBuffer = reinterpret_cast<SArrayBuffer*>(user_alloc(sizeof(SArrayBuffer)-1 + elementSize*maxElements));
	if( newBuffer )
	{
...
void CScriptArray::Resize(int delta, asUINT at)
{
	...
	{
		// Allocate memory for the buffer
		SArrayBuffer *newBuffer = reinterpret_cast<SArrayBuffer*>(user_alloc(sizeof(SArrayBuffer)-1 + elementSize*(buffer->numElements + delta)));
		if( newBuffer )
		{
...
{
	*buf = reinterpret_cast<SArrayBuffer*>(user_alloc(sizeof(SArrayBuffer)-1+elementSize*numElements));
...
void CScriptArray::Precache()
{
	...
	// Create the cache
	cache = reinterpret_cast<SArrayCache*>(user_alloc(sizeof(SArrayCache)));
	if( !cache )
	{
		asIScriptContext *ctx = asGetActiveContext();
		if( ctx )
...
void CScriptArray::Release() const
{
	...
	if( asAtomicDec(refCount) == 0 )
	{
		...
		this->~CScriptArray();
		user_free(const_cast<CScriptArray*>(this));
	}
}
...
static void RegisterScriptArray_Generic(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "void sort_ascending()", asFUNCTION(ScriptArraySortAsc_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sort_ascending(uint startAt, uint count)", asFUNCTION(ScriptArraySortAsc2_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sort_descending()", asFUNCTION(ScriptArraySortDesc_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sort_descending(uint startAt, uint count)", asFUNCTION(ScriptArraySortDesc2_Generic), asCALL_GENERIC); assert( r >= 0 );
